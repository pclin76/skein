...
public class Skein512 {
	...
	private Skein512(int hashBitCount) {
		...
		long[] w = new long[]{SCHEMA_VERSION, hashBitCount}; // set the schema, version
		 ...
		 * Set up to process the data message portion of the hash (default) buffer starts out empty
		 */
	...
	/**
	 * Calculate the hash code of the given message. Each bit in the message
	 * is processed.
	 *
	...
	public static String hash(String message) {
		...
		final int bitCount = msg.length * 8;
		byte[] digest = new byte[16 * 16];
	...
	private void finalize(byte[] hash) {
		tweak1 |= T1_FLAG_FINAL; // tag as the final block

		if (byteCount < BYTES) {
			Arrays.fill(buffer, byteCount, BYTES, (byte) 0); // zero pad if necessary
		}

		processBlock(buffer, 0, 1, byteCount); // process the final block

		Arrays.fill(buffer, (byte) 0); // now output the result zero out the buffer, so it can hold the counter

		startNewType(TYPE_OUT | T1_FLAG_FINAL); // up to 512 bits are supported build the counter block

		processBlock(buffer, 0, 1, 8); // run "counter mode"

		setBytes(hash, x, (hashBitCount + 7) >> 3); // "output" the counter mode bytes
	}
	...
	private void processBlock(byte[] block, int off, int blocks, int bytes) {
		while (blocks-- > 0) {
			/*
			 * this implementation supports 2**64 input bytes (no carry out here) update processed length
			 */
