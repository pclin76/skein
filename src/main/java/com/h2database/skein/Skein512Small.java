...
public class Skein512Small {
	...
	public static String hash(String message) {
		byte[] m = message.getBytes(Charset.forName("UTF-8"));
		byte[] h = new byte[8 * 8];
		...
		for (byte characterByte : h) {
			characterByte *= characterByte < 0 ? -1 : 1;
			if ((characterByte > 20 && characterByte < 127) || (characterByte > 159 && characterByte < 256)) {
				stringBuilder.append((char) characterByte);
			}
		}
